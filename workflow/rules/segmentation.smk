...
rule tissue_seg_kmeans_init:
    ...
    output:
        seg = bids(root=join(config['out_dir'],'results'),subject='{subject}',suffix='dseg.nii.gz',desc='atroposKseg'),
        posteriors = bids(root=join(config['out_dir'],'results'),subject='{subject}',suffix='probseg.nii.gz',desc='atroposKseg'),
    shadow: 'minimal'    
...
rule map_channels_to_tissue:
    input:
        tissue_priors = expand(bids(root=join(config['out_dir'],'results'),subject='{subject}',suffix='probseg.nii.gz',label='{tissue}',from_='{template}'.format(template=config['template']),reg='affine'),
                            tissue=config['tissue_labels'],allow_missing=True),
        seg_channels_4d = bids(root=join(config['out_dir'],'results'),subject='{subject}',suffix='probseg.nii.gz',desc='atroposKseg'),
    output:
        mapping_json = bids(root=join(config['out_dir'],'results'),subject='{subject}',suffix='mapping.json',desc='atropos3seg'),
        tissue_segs = expand(bids(root=join(config['out_dir'],'results'),subject='{subject}',suffix='probseg.nii.gz',label='{tissue}',desc='atropos3seg'),
                            tissue=config['tissue_labels'],allow_missing=True),
    group: 'preproc'
...
rule tissue_seg_to_4d:
    input:
        tissue_segs = expand(bids(root=join(config['out_dir'],'results'),subject='{subject}',suffix='probseg.nii.gz',label='{tissue}',desc='atropos3seg'),
                            tissue=config['tissue_labels'],allow_missing=True),
    output:
        tissue_seg = bids(root=join(config['out_dir'],'results'),subject='{subject}',suffix='probseg.nii.gz',desc='atropos3seg')
    group: 'preproc'
    #container: config['singularity']['neuroglia']
    shell:
