...
if debug:
	...
	isub="078"
	datap=r'/home/greydon/Documents/data/SEEG/derivatives/atlasreg/'
	
	input=dotdict({
		'flo':datap+f'sub-P{isub}/sub-P{isub}_space-T1w_desc-affine_ct.nii.gz',
		'ref':datap+f'sub-P{isub}/sub-P{isub}_T1w.nii.gz'
	})
...
#flo_resamp = image.resample_to_img(flo_img, ref_resamp)
...
# html_view = plotting.view_img(flo_resamp, ref_resamp,
#                               opacity=0.3,cmap='viridis',dim=0,
#                               symmetric_cmap=True,title=title,
#                               cut_coords=[0,0,40])

# html_view.save_as_html(snakemake.output.html)

display = plotting.plot_anat(ref_resamp, display_mode='ortho', draw_cross=False, cut_coords=[0,0,40])
fg_svgs = [fromstring(extract_svg(display))]
display.close()

display = plotting.plot_anat(flo_resamp, display_mode='ortho', draw_cross=False, cut_coords=[0,0,40])
