...
if glob.glob(snakemake.input.shopping_list):
	...
	if any([x.lower()=='label' for x in list(df_elec)]):
		df_elec['Label']=[x.strip() for x in df_elec['Label'].str.replace("’","'")]
...
for _, row_elec in df_elec.iterrows():
	...
	if not any(x in row_elec[list(row_elec.keys())] for x in aborted_lang):
		...
		if errors_data is not None:
			error_idx = []
			if any([x.lower() == 'label' for x in list(row_elec.keys())]):
				if [i for i, x in enumerate(list(errors_data['electrode'].values)) if x.lower() in row_elec['Label'].lower()]:
					error_idx = [i for i, x in enumerate(list(errors_data['electrode'].values)) if x.lower() in row_elec['Label'].lower()]
